...
function buildPortfolio(){
  ...
  const allCats=['All',...new Set(p.items.map(i=>i.category))];
  el('port-filter').innerHTML=allCats.map(f=>`
    <button class="fbtn${f==='All'?' active':''}" data-f="${f}">${f}</button>`).join('');

  function render(filter){
    const items=filter==='All'?p.items:p.items.filter(i=>i.category===filter);
    el('port-grid').innerHTML=items.map((item,i)=>{
      const link=item.instagram||(C.contact&&C.contact.instagram)||'https://www.instagram.com/brighttone_event/';
      return `
      <a class="port-card reveal d${(i%3)+1}" href="${link}" target="_blank" rel="noopener">
        <div class="port-card-bg">
          ${item.image
            ?`<img src="${item.image}" alt="${item.title}" loading="lazy">`
            :`<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%">${icon(getCatIcon(item.category),'style="width:64px;height:64px;stroke:var(--primary);opacity:.1"')}</div>`
          }
        </div>
        ...
        <div class="port-play-btn">
          <div class="port-play-ring">
            <svg viewBox="0 0 24 24" width="26" height="26"><polygon points="8,5 19,12 8,19" fill="white"/></svg>
          </div>
          <span class="port-play-label">Play</span>
        </div>
      </a>`;
    }).join('');
    initReveal();
...
const FALLBACK={
  ...
  contact:{phone:"+250 783 458 808",email:"hello@brighttone.rw",location:"Kigali, Rwanda",instagram:"https://www.instagram.com/brighttone_event/",whatsapp:"https://wa.me/250788000000",event_types:["Concert / Festival","Wedding / Celebration","Corporate Event","Other"],success_message:"Thank you! We'll be in touch within 24 hours."},
